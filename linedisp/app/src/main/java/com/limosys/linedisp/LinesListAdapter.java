...
public class LinesListAdapter extends BaseAdapter {
    ...
    public long getItemId(int position) {
        return listOfLines.get(position).getId();
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        convertView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View currentlySelectedView) {
                if(lastSelectedView != null){
                    setItemInactive(lastSelectedView, Color.parseColor(listOfLines.get(position).getColorRgb()));
                }
                ...
                getCallback().onLineSelected(listOfLines.get(position).getId());
        ...
        String[] descWordsArray = listOfLines.get(position).getDesc().split(" ");
        ...
        lineName.setText(listOfLines.get(position).getName());
        lineDescription.setText(lineDescriptionString);
        lineBonus.setText("Bonus $" + (int) listOfLines.get(position).getIncentiveAmount());
        lineCarsRequired.setText(String.valueOf(listOfLines.get(position).getReqCarsCounter()) + " cars required");
        lineETA.setText("ETA: 12 min");

        // TODO: if line is active - change colors
        setItemInactive(convertView, Color.parseColor(listOfLines.get(position).getColorRgb()));
